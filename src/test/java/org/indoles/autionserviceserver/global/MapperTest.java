...
class MapperTest {
    ...
    @Test
    @DisplayName("경매 엔티티를 판매자 경매 간단 정보로 변환하면 도메인의 정보가 동일하게 전달된다")
    void TransferAuctionEntity_ToSellerSimpleInfo() {
        // given
        LocalDateTime now = LocalDateTime.now();
        Auction auction = Auction.builder()
                .id(1L)
                ...
                .build();
        // when
        SellerAuctionSimpleInfoResponse dto = Mapper.convertToSellerAuctionSimpleInfo(auction);
